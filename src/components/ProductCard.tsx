  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden bg-[#111] rounded-[2rem] border border-brand-border/30 shadow-2xl group-hover:border-brand-accent-glow/20 transition-all duration-500">
        ...
        <div className="absolute inset-x-0 bottom-24 md:bottom-28 px-4 flex flex-col gap-3 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500 z-20">
          <div className="flex gap-3">
            ...
            <motion.button 
              ...
              className="w-14 bg-brand-accent-glow text-black items-center justify-center flex rounded-xl shadow-[0_10px_20px_rgba(197,160,89,0.3)] hover:bg-white transition-colors"
            >
              <Plus size={20} />
            </motion.button>
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
          <div className="flex justify-between items-end gap-4">
            <div className="flex-1 min-w-0">
              <h3 className="text-lg md:text-xl font-display font-black uppercase tracking-tight text-brand-secondary mb-2 truncate group-hover:text-brand-accent-glow transition-colors">
                {product.name}
              </h3>
              <div className="flex items-center gap-2 text-[10px] text-brand-muted font-bold">
                 <div className="flex items-center text-brand-accent-glow">
                    ...
                    <span className="ml-1 text-brand-secondary">{product.rating}</span>
                 </div>
                 <span>({Math.floor(Math.random() * 200 + 50)})</span>
              </div>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-[10px] text-brand-muted line-through opacity-50 mb-1">
                 ৳{(product.price * 1.2 * 110).toFixed(0)}
              </span>
              <span className="text-lg md:text-xl font-black text-brand-accent-glow">
